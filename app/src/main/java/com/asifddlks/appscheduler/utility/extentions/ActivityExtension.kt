 ...
 * Asif Ahmed
 */
fun AppCompatActivity.backgroundThreadCoroutine(work: suspend (() -> Unit)) =
    this.lifecycleScope.launch(
        Dispatchers.IO
    ) { work() }

fun AppCompatActivity.mainThreadCoroutine(work: suspend (() -> Unit)) = this.lifecycleScope.launch(
    Dispatchers.Main
) { work() }

fun AppCompatActivity.defaultThreadCoroutine(work: suspend (() -> Unit)) =
    this.lifecycleScope.launch(
        Dispatchers.Default
    ) { work() }

fun AppCompatActivity.showNotification(contentTitle:String, contentText:String) {
